...
echo "Backend server: http://localhost:8800"
...
stop_servers() {
  ...
  kill -SIGTERM $backend_pid #$action_pid $python_pid
  wait $backend_pid # $action_pid $python_pid  # Wait for processes to exit
